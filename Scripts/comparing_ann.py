...
def init(Set):
    global annotators_dir, pre_processing_dir, post_processing_ann_dir, post_processing_csv_dir, ctakes_dir, all_differences_csv_dir
                ...
                with open(os.path.join(annotators_deep_dir,annotators_files), "r") as r:
                    ...
                    with open(os.path.join(pre_pro, annotators_files),"w") as w:
                        ...
                        for full_line in r:
                            ...
                            if not (line.startswith("HORA") or full_line.startswith("#") or line.startswith("FECHA") or line.startswith("_SUG_") or line.startswith("TIEMPO")):
...
def post_processing(annotators_entities, annotators_notes):
    ...
    for dir, files in annotators_entities.items():
        ...
        for file, records in files.items():
            if file_dic.get(file) == None:
                file_dic[file] = 1
            with open(os.path.join(post_processing_ann_dir, file), "a+") as w_ann:
                for record in records:
                    w_ann.write("T"+ str(file_dic.get(file)) + "\t" + record['label']+" " + str(record['start']) +" " + str(record['end'])+"\t" + record['text'] + "\n")
                    hash = False
                    ...
                    if hash:
                        w_ann.write(
                            "#" + str(file_dic.get(file)) + "\tAnnotatorNotes T" + str(file_dic.get(file)) + "\t" + rec_note[
                                'rest'].strip() + " Annotator: " + dir + "\n")
                    else:
                        w_ann.write(
                            "#" + str(file_dic.get(file)) + "\tAnnotatorNotes T" + str(file_dic.get(file)) + "\t" + "Annotator: " + dir + "\n")
            ...
            if ann_file.get(file) == None:
                ann_records = {}
            else:
                ann_records = ann_file.get(file).copy()

            for record in records:
                temp_key = record["label"] + " " + str(record["start"]) + " " + str(record["end"]) + " " + record["text"]
        ...
        for file in annotators_entities_ant.keys():
            ...
            for cta_ent in ctakes_ents:
                ...
                for ann_ent in annotators_ents:
                    if cta_ent['start'] == ann_ent['start'] and (cta_ent['label'] == "_SUG_"+ann_ent['label'] or ((cta_ent['label'].startswith("FECHA") or cta_ent['label'].startswith("HORA") or cta_ent['label'].startswith("TIEMPO")) and cta_ent['end'] == ann_ent['end'])):
                        remove = False
                if remove:
            ...
            for ann_ent in annotators_ents:
                ...
                for cta_ent in ctakes_ents:
                    if cta_ent['start'] == ann_ent['start'] and cta_ent['end'] == ann_ent['end']:
                        if cta_ent['text'] == ann_ent['text']:
                            ...
                            add = False
                    elif ann_ent['start'] == cta_ent['start'] and cta_ent['label'] == "_SUG_"+ann_ent['label'] and ann_ent['end'] != cta_ent['end']:
                        entity = {}
                        entity['Annotated'] = ann_ent['row']
                        entity['cTAKES'] = cta_ent['row']
                        entity['text'] = ann_ent['text']
                ...
                if add:
                    ...
                    if not( ann_ent['label'].startswith('Hora_')or ann_ent['label'].startswith('Fecha') or ann_ent['label'].startswith('Tiempo_')):
                ...
                with open(os.path.join(analysis_dir, key+".csv"), "w") as w_csv:
                    ...
                    if records:
                        ...
                        for i,record in enumerate(records):
                            if key == "changed" and (record['label'].startswith('Hora_')or record['label'].startswith('Fecha') or record['label'].startswith('Tiempo_')):
                                count_hft+=1
                            elif key == "changed":
                                count_span +=1
...
if __name__ == "__main__":
    ...
    adds, changes, accepts, removes_2, new_variable = statistical_analysis(list_annotators, ctakes_entities, annotators_entities)
